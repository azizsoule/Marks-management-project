...
abstract class Genre implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\GenreTableMap';
    ...
    /**
     * Initializes internal state of Base\Genre object.
     */
     ...
     * @return $this|\Genre The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[GenreTableMap::COL_ID] = true;
        }
    ...
    public function setLibelle($v)
    {
        ...
        if ($this->libelle !== $v) {
            $this->libelle = $v;
            $this->modifiedColumns[GenreTableMap::COL_LIBELLE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : GenreTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : GenreTableMap::translateFieldName('Libelle', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Genre'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[GenreTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . GenreTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(GenreTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(GenreTableMap::COL_LIBELLE)) {
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Genre The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(GenreTableMap::COL_ID)) {
            $criteria->add(GenreTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(GenreTableMap::COL_LIBELLE)) {
            $criteria->add(GenreTableMap::COL_LIBELLE, $this->libelle);
        }
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildGenre The current object (for fluent API support)
     */
     ...
     * @return ObjectCollection|ChildEtudiant[] List of ChildEtudiant objects
     */
    public function getEtudiantsJoinNiveau(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildEtudiantQuery::create(null, $criteria);
        $query->joinWith('Niveau', $joinBehavior);
